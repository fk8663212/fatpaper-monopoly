...
const chanceCardFormRules = reactive<FormRules>({
	...
	teach: [{ required: true, message: "教學模式專用", trigger: "blur" }],
});
...
const handleCreateOrUpdateChanceCard = async (formEl: FormInstance | undefined) => {
	...
	await formEl.validate(async (valid) => {
		if (valid) {
			const { name, describe, type, color, icon, effectCode,teach} = {
				...chanceCardForm,
			};
			if (_chanceCardId) {
				const {
					...
					teach: new_teach,
				} = (await updateChanceCard(_chanceCardId, name, describe, type, color, effectCode,teach, tempIconFile.value)) as any;
				setChanceCardInfo(new_name, new_describe, new_type, new_color, new_icon, new_effectCode,new_teach);
			} else {
				if (!tempIconFile.value) {
					ElMessage({ message: "读取icon文件失败", type: "error" });
					return;
				}
				await createChanceCard(name, describe, type, color, tempIconFile.value, effectCode,teach);
...
const loadChanceCardInfo = async () => {
	if (_chanceCardId) {
		const { name, describe, type, color, icon, effectCode,teach } = (await getChanceCardById(_chanceCardId)) as any;
		setChanceCardInfo(name, describe, type, color, icon, effectCode,teach);
	} else {
		editorVisible.value = true;
	}
};

const uploadAction = computed(() => `${__MONOPOLYSERVER__}/chance_card/${_chanceCardId ? "update" : "create"}`);
		...
		<div class="left-container">
			...
			<div class="chance-card-form-container">
				...
				>
					...
					<el-form-item label="教學模式專用" prop="teach">
						<el-select v-model="chanceCardForm.teach">
							<el-option label="通用" value="0"></el-option>
							<el-option label="跳過題目" value="1"></el-option>
							<el-option label="直接獲得分數" value="2"></el-option>
							<el-option label="更換題目" value="3"></el-option>
						</el-select> 
					</el-form-item>
					...
					<el-form-item label="机会卡图标" prop="icon">
						<el-upload
							...
							accept=".png,.jpg"
							:action="uploadAction"
							:on-exceed="handleIconUploadExceed"
