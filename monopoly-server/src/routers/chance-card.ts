...
routerChanceCard.post("/create", iconUploaderMulter.single("icon"), async (req, res, next) => {
	...
	if (req.file.originalname) {
		...
		const { name, describe, type, color, effectCode ,teach} = req.body;
		if (name && describe && type && color && effectCode != undefined) {
			try {
				await createChanceCard(name, describe, type, fileUrl, color, effectCode,teach);
				const resMsg: ResInterface = {
					status: 200,
					msg: "机会卡创建成功",
				};
...
routerChanceCard.post("/update", iconUploaderMulter.single("icon"), async (req, res, next) => {
	...
	const { id, name, describe, type, color, effectCode,teach } = req.body;
	if (id && name && describe && type && color && effectCode !== undefined) {
		try {
			const resMsg: ResInterface = {
				status: 200,
				msg: "更新机会卡信息成功",
				data: await updateChanceCard(id, name, describe, type, newFileUrl, color, effectCode,teach),
			};
			res.status(resMsg.status).json(resMsg);
		} catch (e) {
			const resMsg: ResInterface = {
