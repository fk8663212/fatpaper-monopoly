...
export const updateChanceCard = async (
	...
	console.log("updateChanceCard", id);
	const chanceCard = await chanceCardRepository.findOne({ where: { id } });
	if (chanceCard) {
		Object.assign(chanceCard, { name, type, describe, color, effectCode,teach });
		if (icon) {
...
export const getChanceCardById = async (id: string) => {
	const chanceCard = await chanceCardRepository.findOne({
		select: ["id", "name", "describe", "type", "icon", "color", "effectCode", "teach"],
		where: { id },
	});
	if (chanceCard) {
		return chanceCard;
