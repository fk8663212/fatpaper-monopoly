...
import {AppDataSource,UserDataSource} from "./src/db/dbConnecter";
...
async function bootstrap() {
	try {
		...
		await UserDataSource.initialize().then(() => {
			serverLog(`${chalk.bold.bgGreen(" 用戶資料庫連接成功 ")}`);
		});
		
		const publicKey = await getPublicKey();
		serverLog(`${chalk.bold.bgGreen(" User server連接成功 ")}`);
