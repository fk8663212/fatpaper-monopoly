...
export class MonopolyClient {
	...
	private messageBoxInstance:any;
			...
			this.conn.on("data", (_data: any) => {
				...
				switch (data.type) {
					...
					case SocketMsgType.BuyProperty:
						...
						break;
					case SocketMsgType.TeachChanceCardUsing:
						this.chanceCardUsing_changeFPMessage(data);
						break;
					...
					case SocketMsgType.ResumeGame:
						...
						break;
					case SocketMsgType.RemainingTime:
						this.handleRemainingGameTime(data);
						break;
					default:
						break;
				}
	...
	private handleRemainingGameTime(data: SocketMessage) {
		useGameInfo().RemainingTime = data.data;
	}
	...
	private handleRoomInfoReply(data: SocketMessage) {
		...
		this.isTeachMode = roomInfoData.gameSetting.teachMode;
		roomInfoData &&
			roomInfoStore.$patch({
	...
	private handleGameInfo(data: SocketMessage) {
		...
		if (gameInfo) {
			gameInfoStore.$patch({
				...
				teachMode : this.isTeachMode,
				RemainingTime:gameInfo.gameTime,
			});
	...
	private handleBuyProperty(data: SocketMessage) {
		...
		if (data.msg?.content === "teach") {
			...
			this.messageBoxInstance=FPMessageBox({
				title: "购买地皮",
				content: vnode,
				cancelText: "@",
				confirmText: "@",
	...
	private chanceCardUsing_changeFPMessage(data: SocketMessage) {
		console.log("確認是否有正確使用機會卡"+data.data);
		if (data.data === "teach") {
			this.messageBoxInstance.visible = false;
			this.sendMsg(SocketMsgType.BuyProperty, OperateType.BuyProperty, undefined, true);
		}
	}
	...
	private handleGameOver(data: SocketMessage) {
		...
		//將資料利用axios傳送到後端
		axios.post("/storeGame/store", null, {
			params: {
