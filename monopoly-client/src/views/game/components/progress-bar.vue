...
const endFraction = computed(() => useRoomInfo().gameSetting.endFraction);

function getBlockHeight(money: number) {
	if(useRoomInfo().gameSetting.endFraction) {
		const height = money / endFraction.value;
		console.log(height);
...
<template>
	<div v-if="!useRoomInfo().gameSetting.teachMode" class="progress-bar">
	...
	<div v-if="useRoomInfo().gameSetting.teachMode" class="progress-bar">
		<div class="target-bar">目标: {{endFraction }}分</div>
		<div
			:style="{ height: getBlockHeight(player.money), backgroundColor: player.user.color }"
			class="player-block"
			v-for="player in playerList"
		...
		>
			<div class="avatar-container" :style="{ '--c': lightenColor(player.user.color, 15) }">
				...
				<div class="money">{{ player.money }}分</div>
			</div>
		</div>
	</div>
</template>
