...
const questionTypes = ref(_tempGameSettingFrom.value.selectedQuestionTypes);
const allQuestions = ref(_tempGameSettingFrom.value.AllQuestions);


axios.get(`/question/list`).then((res) => {
	questionTypes.value = res.data;
	_tempGameSettingFrom.value.ifQuestion = new Array(res.data.length).fill(false);
	//抓取所有的題目
	for (let i = 0; i < res.data.length; i++) {
		axios.get(`/question/detail?filename=${questionTypes.value[i]}`).then((res) => {
			console.log(JSON.stringify(res.data));
			_tempGameSettingFrom.value.AllQuestions.push(JSON.stringify(res.data));
		});
	}
	console.log(_tempGameSettingFrom.value.AllQuestions);
});
...
function handleUpdateGameSetting() {
	console.log(_tempGameSettingFrom.value.ifQuestion);
	if (_tempGameSettingFrom.value.overMoney <= _tempGameSettingFrom.value.initMoney) {
		FPMessage({ type: "error", message: "目标金钱必须大于初始金钱" });
		return;
	}
	if (_tempGameSettingFrom.value.teachMode){
		//確認是否有選擇題目
		if (_tempGameSettingFrom.value.ifQuestion.every((item) => item === false)) {
			FPMessage({ type: "error", message: "請選擇題目" });
			return;
		}
	}
	if (socketClient) {
...
watch(
	() => _tempGameSettingFrom.value.teachMode,
	(newVal) => {
		if (newVal) {
			_tempGameSettingFrom.value.initMoney = 1;
		} else {
			_tempGameSettingFrom.value.initMoney = 2000;
		}
	}
);
	...
	<div class="room-page">
		<div class="left-container">
			...
			<div class="map-option">
				<div v-if="!_tempGameSettingFrom.teachMode" class="options">
				...
				<div v-if="_tempGameSettingFrom.teachMode" class="options">
					<span class="label">目标分數</span>
					<div>
						<input
							:disabled="!isOwner"
							type="number"
							:min="1"
							v-model="_tempGameSettingFrom.endFraction"
						/>分
					</div>
				</div>
				<div v-if="_tempGameSettingFrom.teachMode" class="options">
					<span class="label">初始分數</span>
					<div>
						<input
							:disabled="!isOwner"
							type="number"
				...
				<div v-if="_tempGameSettingFrom.teachMode" class="options">
					<span class="label">對局時間</span>
					<div>
						<input
							:disabled="!isOwner"
							type="number"
							:min="1"
							v-model="_tempGameSettingFrom.endPlayingTime"
						/>分
					</div>
				</div>
				<div class="options">
				...
				<div v-if="_tempGameSettingFrom.teachMode" class="options">
					<span class="label">选择题目种类</span>
					<div class="selected-options" style="display: block;">
						<label v-for="(option, index) in questionTypes" :key="index" class="option" >
							<input type="checkbox" :value="option" v-model="_tempGameSettingFrom.ifQuestion[index]" />
							{{ option }}<br />
						</label>
					</div>
				</div>
